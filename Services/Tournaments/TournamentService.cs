...
namespace Services
{
    public class TournamentService : ITournamentsService
    {
        ...
        public async Task<List<TournamentOutputDto>> GetUserTournaments(int userId, string role)
        {
            if (role == Roles.Organization)
            {
                var orgTournaments = await UnitOfWork.Tournaments.GetUserTournaments(userId);

                return Mapper.Map<List<TournamentOutputDto>>(orgTournaments);
            }

            var tournaments = await UnitOfWork.Tournaments.GetTournamentWithUserParticipation(userId);

            return Mapper.Map<List<TournamentOutputDto>>(tournaments);
        }
        ...
        public async Task<TournamentOutputDto> GetTournamentById(int tournamentId, int userId)
        {
            ...
            if (userTeam != null)
            {
                var team = await UnitOfWork.Teams.GetTeam(userTeam.TeamId);

                if (team != null &&
                    team.Members.Any(m => m.Id == userId && m.IsTeamCaptain) &&
                    participants.Any(p => p.TeamId == team.TeamId))
                {
                    tournamentOutput.CanLeaveTournament = true;
                }
            }

            if (!participants.Any(x => x.TeamId == null) && tournament.UserId == userId)
            {
                tournamentOutput.CanStartTournament = true;
            }

            if (!tournament.HasFinished && tournament.HasStarted)
            {
        ...
        public async Task<List<ErrorModel>> JoinTournament(int tournamentId, int userId, bool allowAccess = false)
        {
            ...
            {
                errors.Add(new ErrorModel { Error = ErrorMessages.TournamentHasStarted });
                return errors;
            }
            ...
            if (!allowAccess && tournament.TournamentAccessId != (int)Core.Enums.TournamentAccess.Public)
            {
        ...
        public async Task<bool> GetUserIsAllowedToParticipate(int tournamentId, int userId)
        {
            ...
            if (await UnitOfWork.Notifications.GetHasNotRequestedToJoinTournamentBefore(userId, tournamentId) != null)
            {
                return false;
            }
        ...
        public async Task<List<ErrorModel>> RequestToJoinTournament(Notification notification, int tournamentId, int userId)
        {
            ...
            if (tournament.HasStarted)
            {
                errorModel.Add(new ErrorModel { Error = ErrorMessages.TournamentHasStarted });
            }
            ...
            notification.EntityId = tournamentId;
            notification.Message = $"<a href='{notification.RedirectUrl}'>{userTeam.Name }</a> wants to join your tournament {tournament.Name}";
        ...
        public async Task<List<ErrorModel>> LeaveTournament(int tournamentId, int userId)
        {
            var errors = new List<ErrorModel>();
            var tournament = await UnitOfWork.Tournaments.GetTournamentById(tournamentId);
            var userTeam = await UnitOfWork.Teams.GetUserTeam(userId);
            ...
            if (!team.Members.Any(m => m.IsTeamCaptain && m.Id == userId))
            {
                errors.Add(new ErrorModel { Error = ErrorMessages.TournamentDoesNotExist });
                return errors;
            }

            if (tournament == null)
            {
                errors.Add(new ErrorModel { Error = ErrorMessages.OnlyTeamCaptain });
                return errors;
            }

            var participant = await UnitOfWork.TournamentParticipants.GetTournamentParticipant(tournamentId, team.TeamId);
            var matchTeams = await UnitOfWork.TournamentMatchTeams.GetTournamentMatchTeamsByTeamId(team.TeamId);
            ...
            matchTeams.ForEach(x => x.Active = false);
            participant.TeamId = null;

            var notification = new Notification
            {
                EntityId = team.TeamId,
                Message = $"{team.Name} has left your tournament '{tournament.Name}'",
                Pending = true,
                NotificationTypeId = (int)NotificationTypes.Alert,
                SenderId = userId,
                ReceiverId = tournament.UserId
            };
        ...
        public async Task<List<ErrorModel>> StartTournament(int tournamentId, int userId)
        {
            var errors = new List<ErrorModel>();
            var tournament = await UnitOfWork.Tournaments.GetByIdAsync(tournamentId);
            var unassigned = await UnitOfWork.TournamentParticipants.FindUnassignedParticipant(tournamentId);
            ...
            if (unassigned != null)
            {
                errors.Add(new ErrorModel
                {
                    Error = ErrorMessages.NotEnoughParticipants
                });
            ...
            if (tournament.HasStarted)
            {
                errors.Add(new ErrorModel
                {
                    Error = ErrorMessages.TournamentHasStarted
                });
